...
class TestObjectInfoAPI(CPCommonTest):
    ...
    def test_objectinfo_api(self):
        """Test the GET method."""
        main()
        req = requests.get('{0}/objectinfo?object_class_name=list'.format(self.url))
        self.assertEqual(req.status_code, 200)
        req_json = loads(req.text)
        self.assertTrue('available_objects' in req_json)
        req_default = requests.get('{0}/objectinfo?object_class_name=Keys'.format(self.url))
        req_explicit = requests.get('{0}/objectinfo?object_class_name=Keys&operation=overview'.format(self.url))
        ...
        req = requests.get('{0}/objectinfo?object_class_name=Keys&operation=hashlist'.format(self.url))
        self.assertEqual(req.status_code, 200)
        req_json = loads(req.text)
    ...
    def test_bad_objectinfo_api(self):
        ...
        req = requests.get('{0}/objectinfo?object_class_name=DoesNotExist'.format(self.url))
        self.assertEqual(req.status_code, 404)
        self.assertTrue("'DoesNotExist' is not a valid class object name" in req.text)
